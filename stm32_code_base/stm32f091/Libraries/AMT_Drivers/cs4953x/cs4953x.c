...
#warning "these definition when DSP deliverable file is confirmed, the flow should be modified"
#define config_SUPPORT_AC3 0
#define config_SUPPORT_DTS 0
#define config_SUPPORT_PCM_PROCESSING_MODE 0
#define config_SUPPORT_PCM_MC 0
#define config_SUPPORT_SURROUND_MODULE 0
...
#define RS_G2   ( CS4953x_CMD_BASE + _g_0_Ls_Rs_Gain_2157 )
#define LS_G2   ( CS4953x_CMD_BASE + _g_1_Ls_Rs_Gain_2157 )
...
#define SUBWOOFER_G1 ( CS4953x_CMD_BASE + _g_0_Subwoofer_Gain_1_1161 )

// gain_multi_channel_friendly_plus24 (Subwoofer Gain 2)
#define SUBWOOFER_G2 ( CS4953x_CMD_BASE + _g_0_Subwoofer_Gain_2159 )
...
static int CS4953x_readPresence( uint8 *pValue )
{
    ...
    uint32 command = 0;
    uint32 value = 0;
    uint32 LeftChannelSignal = 0;
    uint32 RightChannelSignal = 0;
...
static bool cs4953x_spi_mutex_take( void )
{
    if ( pBSP_SPI_OBJECT->dsp_spi_IsEnable() != TRUE )
    {
...
#if( config_SUPPORT_SURROUND_MODULE == 0 )
            ...
            case LOADER_SOFTBOOT:
                {
                    ...
                    do
                    {
                        while ( pBSP_GPIO_OBJECT->read( __I_SCP1_IRQ ) == 0 )
                        {
                            CS4953xSPI_read_buffer((byte *)&value,sizeof(value)) == SCP1_IRQ_TIMEOUT;
                            TRACE_DEBUG((0, "value = 0x%X", value));
                        }
...
#if ( config_SUPPORT_PCM_PROCESSING_MODE == 1 )
                        if ( pMode->srs_tshd )
                        {
                            if ( pMode->srs_truvol )
                            {
...
#if ( config_SUPPORT_AC3 == 0 )
#warning "TODO: AC3 stream deliverable file is not ready "
#else
    switch ( module )
    {
        case GAIN_ULD:
...
#if ( config_SUPPORT_DTS == 0 )
#warning "TODO: DTS stream deliverable file is not ready "
#else
    switch ( module )
    {
        case GAIN_ULD:
...
static int8 cs4953x_Load_DSP_PCM_ULD_InternalFlash( cs495x_ULD_Type module )
{
    ...
    switch ( module )
    {
        ...
        case OS_ULD:
        {
            if ( cs4953x_spi_mutex_take() == TRUE )
            {
                if (CS4953xSPI_write_buffer((byte *)&OS_HD2_RC_X_ULD, (sizeof(OS_HD2_RC_X_ULD)/sizeof(uint8)) ) == SCP1_BSY_TIMEOUT )
                {
                    ret = -1;
                }
