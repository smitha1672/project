...
#define DEBUG_MSG_WITH_TIME_ENABLE 0
...
#if DEBUG_MSG_WITH_TIME_ENABLE // Jackson, add time in debug info.
    Debug_writeString("\t<<  ");
    uint32 timeMs = VIRTUAL_TIMER_TICKS2MSEC(VirtualTimer_now());
    uint16 ms = timeMs % 1000;
    uint16 sec = timeMs / 1000;
    char timeStr[20];
	sprintf(timeStr, "%d:%03d", sec, ms);
    Debug_writeString(timeStr);
    Debug_writeString("  >>  ");
#else
    Debug_writeString("\t>>  ");
#endif
