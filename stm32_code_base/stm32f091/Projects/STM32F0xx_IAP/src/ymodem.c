...
#ifdef EXT_FLASH_UPDATE_APP
#define MCU_APP_SIZE             (uint32_t)0x3cffe
#define DSP_VERSION_SIZE         (uint32_t)0x06
...
#ifdef EXT_FLASH_UPDATE_APP  
  flashdestination = EXT_FLASH_MCU_VER_START;
#elif defined EXT_FLASH_UPDATE
  flashdestination = 0;
#else
...
#ifdef EXT_FLASH_UPDATE_APP
                  if (flashdestination >= EXT_FLASH_MCU_VER_END)
                  {
                      flashdestination = 0;
                  }
#endif
...
#ifdef EXT_FLASH_UPDATE
                  if (sFLASH_WriteBuffer((uint8_t*) ramsource,&flashdestination, (uint16_t) packet_length)  == 0)
#else
