...
#define USBH_CORE_100MS TASK_MSEC2TICKS(100)

portTickType duration;
int16_t HostDeviceCount = 0;
               ...
               USBH_Usr_cb_TypeDef *usr_cb)
{
  ...
  USB_OTG_BSP_Host_EnableInterrupt(pdev); /*Smith JUNE 14 2013: When USB is set to HOST mode, its priority should be higher than timer.*/
}
...
* @retval None
*/
HOST_State USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_IDLE :
	...
	else
	{
        ...
        xTimeTick = xTaskGetTickCount( );
        if ( (duration/portTICK_RATE_MS) > TASK_MSEC2TICKS(10) )
        {
            HostDeviceCount ++;
        }
        
		/*Smith: Don't modify HostDeviceCount that interfere USB flash driver compatibility*/
		if ( HostDeviceCount >= 220 ) 
		{
			HostDeviceCount = 0;
  ...
  default :
	...
	return phost->gState; /*smith implemented*/
}
