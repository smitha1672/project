...
int8_t if_initInterface(hwInterface* file, char* opts)
{
  ...
  if(HCD_IsDeviceConnected(&USB_OTG_Core))  //Jerry change
  {  
...
int8_t if_readBuf(hwInterface* file,uint32_t address,uint8_t* buf)
{
  ...
  {  
    ...
    while((status == USBH_MSC_BUSY ) && (HCD_IsDeviceConnected(&USB_OTG_Core)));//(HCD_IsDeviceConnected(&USB_OTG_FS_dev)));
...
int8_t if_writeBuf(hwInterface* file,uint32_t address,uint8_t* buf)
{
  ...
  {  
    ...
    while((status == USBH_MSC_BUSY ) && \
      ...
      (HCD_IsDeviceConnected(&USB_OTG_Core)));  //Jerry change
  }
  return(status);
}
...
int8_t if_TestUnitReady(void)
{
  ...
  {  
    do
    {
      status = USBH_MSC_TestUnitReady(&USB_OTG_Core);//USBH_MSC_TestUnitReady();
      //USBH_MSC_HandleBOTXfer();
      USBH_MSC_HandleBOTXfer(&USB_OTG_Core, &USB_Host);
    }
    while((status == USBH_MSC_BUSY ) && \
