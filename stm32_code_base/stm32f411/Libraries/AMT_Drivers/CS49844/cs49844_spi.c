...
/*-------------------------------------------------------------------------------------------*/
#define SLAVE_ADDRESS_W_CS49844	0x80
#define SLAVE_ADDRESS_R_CS49844	0x81
...
int8 CS49844SPI_write_buffer(const byte* data, uint16 length)
{
    ...
    /* Select SPI chip.*/
    SPI_selectChip(CS49844_SPI_NPCS);
    SPI_write( SLAVE_ADDRESS_W_CS49844 );
    ...
    SPI_unselectChip(CS49844_SPI_NPCS);
...
int8 CS49844SPI_read_buffer( byte* data, uint16 length )
{
    ...
    if (ret_val != SCP1_BSY_TIMEOUT)
    {
        ...
        SPI_write(SLAVE_ADDRESS_R_CS49844);
...
int8 CS49844SPI_MsgNote1( byte* data, uint16 length )
{
    ...
    /* Select SPI chip.*/
    if (SPI_selectChip(CS49844_SPI_NPCS) == FALSE)
    {
        ret_val = SCP1_BSY_TIMEOUT;
        //return ret_val;
    }
...
bool CS49844SPI_CommandWrite(uint32 cmd, uint32 value)
{
    ...
    if (CS49844SPI_write_buffer((byte*)&command, (sizeof(command)/sizeof(uint8))) == SCP1_IRQ_TIMEOUT )
    {
        return FALSE;
    }
        
    if (CS49844SPI_write_buffer((byte*)&val, (sizeof(val)/sizeof(uint8))) == SCP1_IRQ_TIMEOUT )
    {
        return FALSE;
    }
...
bool CS49844SPI_ReadSolicited( uint32 cmd, uint32 *pValue )
{
    ...
    if (CS49844SPI_read_buffer((byte*)&value, sizeof(value)/sizeof(uint8))== SCP1_IRQ_TIMEOUT)
    {
        return FALSE;
    }
