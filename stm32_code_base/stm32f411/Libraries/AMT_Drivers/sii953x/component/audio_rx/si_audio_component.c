	...
	if (SiiDrvRxAudioIsInterruptAssert())
	{
	    ...
	    else 
	    {
			...
			//	DEBUG_PRINT( MSG_ALWAYS, "=== FNCHG ===\n");
				...
				pAudExtraction->audMode = SiiDrvRxAudioModeGet();
		...
		pAudExtraction->audMode = SiiDrvRxAudioModeGet();
		pAudExtraction->audLayout = SiiDrvRxAudioLayoutGet();
		pAudExtraction->audFs = SiiDrvRxAudioSampleRateGet();
		pAudExtraction->audMclk = SiiDrvRxAudioGetMclk();
