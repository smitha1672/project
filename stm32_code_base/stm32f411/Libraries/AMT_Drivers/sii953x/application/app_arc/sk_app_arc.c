...
static bool_t ArcSendResponse ( uint8_t opCode, int portIndex, int destLA, bool_t isArcTx )
{
	...
	else
	{
		...
		//All is well, send the response
	        SiiCecSendMessageTask( opCode, destLA );
#endif
        success = true;
        break;
    }
...
{
    if ( SYS_CEC_TASK & SYS_CEC_TASK_ARC_INITIONAL )
    {	
	//CecInitialARCSend();
    ...
    else if ( SYS_CEC_TASK & SYS_CEC_TASK_ARC_TERMINATE )
    {
	//CecTerminalARCSend();
	AmTARCMessageSend(FALSE);
	ArcTaskCounter = 1;
		
	SYS_CEC_TASK &= !(SYS_CEC_TASK_ARC_TERMINATE);
    }
    else if ( SYS_CEC_TASK & SYS_CEC_TASK_ARC_REPORT_INITIONAL )
    {
	SiiDrvArcConfigureArcPin( SiiCecInstanceGet(), SiiCecInstanceGet(), ARC_RX_MODE );
	ArcTaskCounter = 0;
	
	SYS_CEC_TASK &= !(SYS_CEC_TASK_ARC_REPORT_INITIONAL);
    }
    else if ( SYS_CEC_TASK & SYS_CEC_TASK_ARC_REPORT_TERMINATE )
    {
	SiiDrvArcConfigureArcPin( SiiCecInstanceGet(), SiiCecInstanceGet(), ARC_DISABLE_MODE );
	ArcTaskCounter = -1;
	
	SYS_CEC_TASK &= !(SYS_CEC_TASK_ARC_REPORT_TERMINATE);
    }
    ...
    else if ( ArcTaskCounter == -1)
    {
            xHMISystemParams SysPrams;

            SysPrams = AmTCecGetSyspramsFromHMI();
			
    	if ( SysPrams.input_src == USER_EVENT_SEL_HDMI_ARC)
    	{
	    SYS_CEC_TASK |= SYS_CEC_TASK_ARC_INITIONAL;
    	}
	else
...
//! @brief      
//-------------------------------------------------------------------------------------------------
void AmTArcTaskAssign( uint8_t SysArcAppTask)
{
	SYS_CEC_TASK |= SysArcAppTask;		
//    TRACE_DEBUG((0, "SYS_CEC_TASK %d", SYS_CEC_TASK ));
		}
//-------------------------------------------------------------------------------------------------
//! @brief      
//-------------------------------------------------------------------------------------------------
