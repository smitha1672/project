...
static void TAS5727_initialization_LR( bool Address );

static void TAS5727_Master_Vol_Mute_LR(bool Address,bool enable );
...
void TAS5727_LR_DRC_enable(bool Address,bool bEnable )
{
    ...
    //DRC
    TAS5727_I2C_write_n_byte(Address,DRC_CTRL_REG, Drc_ctrl, 4);
}
...
static void TAS5727_Master_Vol_Mute_LR(bool Address,bool enable )
{
    if ( enable == TRUE )
    {
        TAS5727_I2C_write(Address,SOFT_MUTE_REG, 0x07);
        TAS5727_I2C_write_n_byte(Address,MASTER_VOL_REG,m_Master_Vol_mute, 2); /*smith fixes*/
    }
    else
    {
        TAS5727_I2C_write_n_byte(Address,MASTER_VOL_REG,m_Master_Vol,2);
        TAS5727_I2C_write(Address,SOFT_MUTE_REG, 0x00);
    }
}

void TAS5727_shutdown(bool Address,bool value )
{
    if ( value == TAS5727_ENTER_SHUTDOWN)
    {
        TAS5727_I2C_write(Address,SYS_CONTROL_2_REG, TAS5727_ENTERSHUTDOWN); /*Enter all-channel shutdown (hard mute)*/
        vTaskDelay(TASK_MSEC2TICKS(165));
    }
    else
    {
        TAS5727_I2C_write(Address,SYS_CONTROL_2_REG, TAS5727_EXITSHUTDOWN); /*Exit all-channel shutdown (hard mute)*/
        vTaskDelay(TASK_MSEC2TICKS(165));
    }
}
...
static void TAS5727_initialization_LR( bool Address )
{
    ...
    TAS5727_I2C_write(Address,OSC_TRIM_REG, 0x00);
    vTaskDelay(TASK_MSEC2TICKS(50));

    //Input Mux Register(BD mode,opt.)
    
    TAS5727_I2C_write_n_byte(Address,PWM_MUX_REG,m_PwmMux_Data,4);    
    
    TAS5727_I2C_write_n_byte(Address,MASTER_VOL_REG,m_Master_Vol_mute,2);
    TAS5727_I2C_write_n_byte(Address,CHANNEL_1_VOL_REG, m_CHANNEL_1_VOL_REG, 2); //0dB
    TAS5727_I2C_write_n_byte(Address,CHANNEL_2_VOL_REG, m_CHANNEL_2_VOL_REG, 2); //0dB
    TAS5727_I2C_write_n_byte(Address,CHANNEL_3_VOL_REG, m_CHANNEL_3_VOL_REG, 2); //0dB
    
    TAS5727_I2C_write(Address,SOFT_MUTE_REG, 0x07);
    TAS5727_I2C_write(Address,SYS_CONTROL_1_REG, 0x80);
    TAS5727_I2C_write(Address,SERIAL_DATA_REG, 0x05);
    TAS5727_I2C_write(Address,SYS_CONTROL_2_REG, TAS5727_ENTERSHUTDOWN);

    TAS5727_I2C_write(Address,VOL_CONFIG_REG, 0x91);
    TAS5727_I2C_write(Address,MODULATION_LIMIT_REG, 0x02);
    TAS5727_I2C_write(Address,START_STOP_PERIOD_REG, 0x0a);
    TAS5727_I2C_write(Address,BKND_ERR_REG, 0x02);
    TAS5727_I2C_write(Address,IC_DEALY_CH_1_REG, TAS5727_ICD1);
    TAS5727_I2C_write(Address,IC_DEALY_CH_2_REG, TAS5727_ICD2);
    TAS5727_I2C_write(Address,IC_DEALY_CH_3_REG, TAS5727_ICD3);
    TAS5727_I2C_write(Address,IC_DEALY_CH_4_REG, TAS5727_ICD4);
    TAS5727_I2C_write_n_byte(Address,INPUT_MUX_REG,m_inputMux_Data,4); 
    
    TAS5727_shutdown(Address,TAS5727_EXIT_SHUTDOWN);
