...
const static uint8 avdelay[]={2,5,8,11,14,17,20,23,26,29,32,35};
#else
const static uint8 avdelay[]={0,2,3,5,6,8,9,11,12,14,15,17,18,20,21,23,24,26,27,29,30,32,33,35};
#endif

const static uint8 LED_SLIDE_START[]={0,0,3,3,6,6,9, 9, 12,12,15,15,15,18,18,21,21,24,24,27,27,30,30,33,33};
const static uint8 LED_SLIDE_END[] = {2,1,5,4,8,7,11,10,14,13,17,16,20,19,20,22,23,25,26,28,29,31,32,34,35};
...
void UIDeviceManager_SetStatus( const void *parms )
{
    ...
    switch(pSystemParms->sys_event)
    {
        ...
        case SYS_EVENT_VOLUME_MUTE_SET:
        {
            ...
            mUIDevParms.master_gain = pSystemParms->master_gain;    //Angus added for setting volume val when mute sb adter demo mode timeout. 
        }
            break;
...
static void UIDeviceManager_handleCommand( const void *parms )
{
    ...
    switch( pSystemParms->sys_event )
    {
        ...
        {
            Last_UIExcept = UI_EVENT_NONE;/*Tony150201:No BT search when power on*/
        ...
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->bass_gain] ,LED_SLIDE_END[pSystemParms->bass_gain],Hold1_5sec,TRUE,TRUE,TRUE);          
        }
            break;

        case SYS_EVENT_TREBLE_SET:
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->treble_gain] ,LED_SLIDE_END[pSystemParms->treble_gain],Hold1_5sec,TRUE,TRUE,TRUE);  
        }
            break;
            
        case SYS_EVENT_SUB_SET:
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->sub_gain] ,LED_SLIDE_END[pSystemParms->sub_gain],Hold1_5sec,TRUE,TRUE,TRUE);            
        }
            break;

        case SYS_EVENT_CENTER_SET:
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->center_gain] ,LED_SLIDE_END[pSystemParms->center_gain],Hold1_5sec,TRUE,TRUE,TRUE); 
        }
            break;
            
        case SYS_EVENT_LSRS_SET:
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->LsRs_gain] ,LED_SLIDE_END[pSystemParms->LsRs_gain],Hold1_5sec,TRUE,TRUE,TRUE); 
        }
            break;
                
        case SYS_EVENT_BALANCE_SET:
        {
            pLED_ObjCtrl->FadeToDarkConverter( LED_SLIDE_START[pSystemParms->balance] ,LED_SLIDE_END[pSystemParms->balance],Hold1_5sec,TRUE,TRUE,TRUE); 
        }
            break;
        ...
        case SYS_EVENT_SYSTEM_RESET:
        {
            ...
            pLED_ObjCtrl->Flashing(UI_VOL_START, UI_VOL_END,18, 3,FALSE);   
        }
            break;
...
static void UIDeviceManager_Exception_handleCommand( xUIExceptionEvents UIExcept)
{
...
#endif
    /*Tony150119:Show vol when enter to demo mode*/
    if(UIExcept == UI_EVENT_ENTER_DEMO1 || UIExcept == UI_EVENT_ENTER_DEMO2 || 
          UIExcept == UI_EVENT_ENTER_DEMO3)
    {
        pLED_ObjCtrl->UpdateDemoShowVolState(TRUE);
    }
    ...
    switch( UIExcept )
    {
        ...
        {
            mUIDevParms.mute = FALSE;  //In demo mode, Sound bar leaves timeout the sound will be unmuted. Angus 2014/11/13  
            pLED_ObjCtrl->LightConverter(UI_VOL_START, 0);
        }
        break;
