...
#define EEPROM_DMA TRUE
#define EEPROM_RETRY_TIME 3
/*------------------------ Private Variables ---------------------------------*/
...
/*----------------------------------------------------------------------------*/
static bool EepromDriver_I2C_Write(byte control, uint16 address, byte *data, uint16 length )
{
    bool ret = TRUE;
    uint8 i;
	...
	/*When this pin was set to high, EEPROM will be released*/  
    GPIOMiddleLevel_Set( __O_MCU_EEPROM_WP ); 

    /*Smith fixes: system hang up issue. The countermeasure follows AT24C04D 8.4  write protect setup time and hold time min is 500ns*/
    vTaskDelay( 1 ); 
    
    for( i = 0; i < EEPROM_RETRY_TIME; i++ )
    {
        ret = I2C_writeSlave(control, address, data, length, FALSE, EEPROM_DMA);
        vTaskDelay( TASK_MSEC2TICKS(I2C_INTERVAL_TIME) );
        if ( ret == TRUE )
        {
        	/*When this pin was set to low, EEPROM will be blocked*/ 
            GPIOMiddleLevel_Clr( __O_MCU_EEPROM_WP ); 
    ...
    if ( ret == FALSE )
    {
		...
		/*Smith fixes: system hang up issue. The countermeasure follows AT24C04D 8.4  write protect setup time and hold time min is 500ns*/
		vTaskDelay( 1 );
		TRACE_ERROR((0, "EEPROM WRITING ERROR device address = 0x%X; register addr = 0x%X; length = 0x%X", control, address, length));
    }
                                   ...
                                   bool verify)
{
    ...
    if ( chunkFreeSize < 0 )
    {
        ...
        if ( NeedchunkNum <= 1 )
        {
            ...
            retVal = EepromDriver_I2C_Write(deviceAddress, (address+writeIndexFront-offset), (buff+writeIndexFront), writeSize );
            if ( retVal == TRUE )
            {
    ...
    {
        deviceAddress = m_logicalToPhysicalEeprom[device].I2CAddress;    
        retVal = EepromDriver_I2C_Write(deviceAddress, (address+writeIndexFront), buff, size);
        if ( retVal == FALSE )
        {
            return retVal;
        }
