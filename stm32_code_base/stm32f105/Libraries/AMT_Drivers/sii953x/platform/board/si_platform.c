...
#if ( configSII_DEV953x_PORTING_PLATFORM_STM32 == 1 )
...
bool_t SiiPlatformInitialize ( void )
{
    ...
    for ( i = 6; i >= 0; i-- )
    {
     ...
     if(platform.semaHardwareInt == NULL)
     {
         platform.semaHardwareInt = xSemaphoreCreateBinary();
         ASSERT(platform.semaHardwareInt != NULL);
         xSemaphoreTake(platform.semaHardwareInt, 0);
     }
#endif 
...
void SiiPlatformInterruptHandler ( void )
{
    ...
    xSemaphoreGiveFromISR(platform.semaHardwareInt, &xHigherPriorityTaskWoken);
#endif 
...
bool_t SiiPlatformInterruptWait ( uint8_t mSec )
{
    platform.hardwareInt = (xSemaphoreTake(platform.semaHardwareInt, TASK_MSEC2TICKS(mSec))==pdPASS);
    return platform.hardwareInt;
}
...
void SiiPlatformInterruptSet( void )
{
    xSemaphoreGive(platform.semaHardwareInt);
}
