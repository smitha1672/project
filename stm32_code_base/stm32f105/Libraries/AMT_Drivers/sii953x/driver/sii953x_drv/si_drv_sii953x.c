...
/*Recover NVRAM data losing conutermeasure for HDMI5 audio drop issue without TV, Smith*/
#define NVRAM_DATA_RECOVER 1
...
#if ( NVRAM_DATA_RECOVER == 1 )
uint8_t SiiDrvDeviceNonVolatile_DataVerify( void )
{
    uint8_t data_error_block = 0xff;
    int16_t i;
    uint8_t verify_step = 0;
    uint8_t *pBootData = (uint8_t *)&gEdidFlashDevBootData;
    uint8_t *pEdidData = (uint8_t *)&gEdidFlashEdidTable[0];

    for( verify_step = 0; verify_step <= 1; verify_step ++ )
    {
        switch( verify_step )
        {
            case 0: /*Check boot data */
            {
                memset( dispData, 0, sizeof(dispData));
                if ( SiiDrvNvramNonVolatileRead( NVRAM_BOOTDATA, dispData, 0, EDID_DEVBOOT_LEN ) == true )
                {
                    // compare boot data
            ...
            {
                memset( dispData, 0, sizeof(dispData));
                if ( SiiDrvNvramNonVolatileRead( NVRAM_HDMI_EDID, dispData, 0, EDID_TABLE_LEN ) == true )
                {
                    // compare boot data
                    for(i = 0; i < EDID_TABLE_LEN; i++ )
                    {
                        if( dispData[i] != pEdidData[i] )
                        {
                            data_error_block = verify_step;
                            break;
                        }
        ...
        if ( data_error_block != 0xff )
            break;    

    }

    return ( data_error_block );
       ...
       // Bug 33154 - Melbourne SiI9533 EDID ID Product Code is NOT correct
    	...
    	if(success)
    	{
			...
			if(!bootDataMismatch)      // Bug 33154 - Melbourne SiI9533 EDID ID Product Code is NOT correct
			{
				...
				{
#if( NVRAM_DATA_RECOVER == 1 )
                    /*if EDID is lost from NVRAM, that is able to recover as below*/
                    for(i = 0; i < EDID_TABLE_LEN; i++ )
                    {
                        if( dispData[i] != pEdidData[i] )
                        {
...
#else
                    /*The judgement condition that can not find data mismatch is too weak, Smith marks*/
                    //compare Edid Data 
...
#if ( NVRAM_DATA_RECOVER == 1 ) /*if NVRAM boot data and EDID are different with CPU default value, here will reload NVRAM 6 times. Smith*/
    do
    {
        if ( !SiiDrvDeviceNonVolatileInitialize( recoverNVRAM ) )
        {
            break;
        }
        
        if ( SiiDrvDeviceNonVolatile_DataVerify() != 0xFF )
        {
            recover_times ++;
        }
        else
...
#if 0 /*Fix sil9533 interrupt pin always keeps low issue, Smith*/
    if( ( intStatus == 0 ) && ( intStatus1 == 0 ) )
    {
