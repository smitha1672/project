...
static uint8_t CecSwitchInputPortGet( uint16_t physAddr )
{
    ...
    {
        nmb = (uint8_t)(physAddr & 0x000F);
    }
    else if ( pa & 0x0F00 )
    {
        nmb = (uint8_t)((physAddr & 0x00F0) >> 4)/* + 1*/;
    }
    else if ( pa & 0xF000 )
    {
        nmb = (uint8_t)((physAddr & 0x0F00) >> 8);
    }
    ...
    if (pCecMessage != 0) //decode a message, update status
    {
        ...
        if (destLogAddr == 0xF) // ignore non-broadcast messages
        {
            switch (pCecMessage->opcode) // CEC operation code
            {
                ...
                case CECOP_STANDBY:
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_STANDBY"));
...
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_ROUTING_CHANGE"));
                    if (pCecMessage->argCount == 4)
                    {
                        ...
                        // power on device when TV switch to SB port, kaomin
                        if(AmTCecGetSystemPowerStatus() == POWER_OFF)
                            AmTCecEventPassToHMI(USER_EVENT_POWER);
                        }
                    }
                    else
                    {
...
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_ROUTING_INFORMATION"));
...
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_ACTIVE_SOURCE"));
#if ( configSII_DEV_953x_PORTING == 0 )
                    if (pCecMessage->argCount != 2)
                    {
...
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_SET_STREAM_PATH"));
                     if(pCecMessage->argCount == 2)
                     {
                         newPhysAddr = (pCecMessage->args[0] << 8) | pCecMessage->args[1];
                         DEBUG_PRINT(MSG_STAT, ("Physical Address : 0x%x.\n"),newPhysAddr);
						 
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_SET_STREAM_PATH: Physical Address :  %X", newPhysAddr));
#if ( configSII_DEV_953x_PORTING == 1 )
                        if ( CecIfChild(newPhysAddr) == TRUE )
			{
                            inputPortIndex = CecSwitchInputPortGet(newPhysAddr);
                        // power on device once tv switch sb by cec name, kaomin
                        if(AmTCecGetSystemPowerStatus() == POWER_OFF)
                            AmTCecEventPassToHMI(USER_EVENT_POWER);	//Power SB on and change to HDMI_ARC
			}
			else
			{
			    inputPortIndex = 0xFF;
			}
TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_SET_STREAM_PATH: inputPortIndex %X", inputPortIndex));
#else
                        inputPortIndex = CecSwitchInputPortGet(newPhysAddr);
#endif
...
//                        if (((pCecMessage->srcDestAddr & 0xF0) >> 4) != CEC_LOGADDR_TV)
                        	...
                        	pCec->activeSrcLogical  = 16;
			        for ( i = 0; i < 16; i++ )
			        {
			            if ( newPhysAddr == pCec->logicalDeviceInfo[i].cecPA )
			            {
                        	        pCec->activeSrcLogical = i;
					break;
			            }
			        }
                            	pCec->activeSrcPhysical = newPhysAddr;

TRACE_DEBUG((0, "CecSwitchTaskBase: CECOP_SET_STREAM_PATH: pCec->activeSrcLogical: %X, activeSrcPhysical: %X", pCec->activeSrcLogical, pCec->activeSrcPhysical));
                            	SiiCecUpdateLogicalDeviceInfo( pCec->activeSrcLogical, pCec->activeSrcPhysical, true );
...
#if ( configSII_DEV_953x_PORTING == 1 )
			 else if ( SiiCecGetDevicePA() == newPhysAddr )
			 {
                            AmTCecEventPassToHMI(USER_EVENT_CEC_POWER_ON);	//Power SB on
                            pCec->activeSrcLogical = CEC_LOGADDR_AUDSYS;
                            pCec->activeSrcPhysical = newPhysAddr;
