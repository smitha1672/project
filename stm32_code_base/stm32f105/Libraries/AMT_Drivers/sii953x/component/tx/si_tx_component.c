...
void SiiTxInitialize(void)
{
    ...
    pTx->status.isSinkReady = true; // setsink ready to fix no sound  when AC on with tv standby, kaomin 
    ...
    pTx->isBadEdidRead = false;
    pTx->isBadEdidTrig = false;
}
...
txStatus_t SiiTxStatusGet(void)
{
    ...
    if(pTx->isBadEdidRead)
        pTx->status.isHotPlugDetected = 0;
    else
        pTx->status.isHotPlugDetected = ((status & SI_TX_STATUS_MASK_HPD) != 0);
    pTx->status.isRxSensed = ((status & SI_TX_STATUS_MASK_RX_SENSE) != 0);

    // this caused SAC abnormal when TV in standby or non SB port, kaomin
    // force HPD low while no Rx sense to fix TV DC off no sound issue, kaomin
    //if(!pTx->status.isRxSensed)
    //    pTx->status.isHotPlugDetected = pTx->status.isRxSensed;
...
txFeedbackMsg_t SiiTxTaskProcess(time_ms_t elapsedTimeMs)
{
    ...
    if(pTx->status.isHotPlugDetected==0 && pTx->status.isRxSensed==1)
    {
        uint8_t plugstatus;
        plugstatus = SiiRegRead(REG_TPI__INT_STATUS1);
        if(plugstatus&BIT_TPI__INT__HPD_STATE)
        {
            SiiDrvTpiSetPlugStatus(true, true);
            txInterruptWord |= SI_TX_IRQ__HPD;
        }
    }
    ...
    if ((txInterruptWord & (SI_TX_IRQ__HPD | SI_TX_IRQ__RX_SENSE))    ||
            ...
            ((!pTx->status.isSinkReady) && pTx->status.isDsConnected) || (pTx->isBadEdidTrig))
    {
        txStatus_t txStat = SiiTxStatusGet();

        //TRACE_DEBUG((0, "kaomin in SiiTxTaskProcess: sinkready=%d dsconnect=%d hpd=%d rxsense=%d", pTx->status.isSinkReady, pTx->status.isDsConnected, pTx->status.isHotPlugDetected, pTx->status.isRxSensed));

        if(pTx->isBadEdidTrig)
        {
            pTx->isBadEdidTrig = false;
        }
        if(txInterruptWord & SI_TX_IRQ__HPD)
        {
            pTx->isBadEdidRead = false;
        }
        ...
        #if 0// this causes no audio issue when vizio p series tv standby, kaomin
        ...
        if (pTx->status.isSinkReady)
        {
            ...
            {
                // SINK READY State
                if (!txStat.isHotPlugDetected/*isRxSensed*/)
                {
        ...
        else
        {
            ...
            {
                // DISCONNECTED State
                if (txStat.isHotPlugDetected/* && txStat.isRxSensed*/) // add Rx sensed to fix no sound  when AC on with tv standby, kaomin 
                {
...
bool_t SiiTxIsHotplugDetected(void)
{
    return (pTx->status.isHotPlugDetected?true:false);
}
bool_t SiiTxIsRxSensed(void)
{
    return (pTx->status.isRxSensed?true:false);
}

void SiiTxSetBadEdidTrig(bool_t bBadEdid)
{
    pTx->isBadEdidTrig = bBadEdid;
    pTx->isBadEdidRead = bBadEdid;
}

void SiiTxSetDsConnect(bool_t bConnect)
{
    pTx->status.isDsConnected = bConnect;
}
