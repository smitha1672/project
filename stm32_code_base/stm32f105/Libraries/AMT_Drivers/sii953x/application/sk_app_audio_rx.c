...
#if ( configSII_DEV_953x_PORTING == 1 )
//when standby mode, we don't need to send the message to audio device manager
                                if (pPowerHandle_ObjCtrl->get_power_state() == POWER_ON)
                                {
       				/*TODO: Send MC-PCM request*/
				if ( RxAudState[0].audLayout == SII_AUD_MULTI_CHANNEL )
				{
					...
					adm_q_event = ADM_QUEUE_EVENT_FROM_HDMI_MC_PCM_REQUEST;
					pADM_QueueObj->SignalDetector_sender( &adm_q_event );
				}
				else if ( RxAudState[0].audLayout == SII_AUD_TWO_CHANNEL )
				{
					adm_q_event = ADM_QUEUE_EVENT_FORM_DIGITAL_SOURCE_CHECK_PLL;
					pADM_QueueObj->SignalDetector_sender( &adm_q_event );

					adm_q_event = ADM_QUEUE_EVENT_FROM_HDMI_2CH_PCM_REQUEST;
					pADM_QueueObj->SignalDetector_sender( &adm_q_event );
				}
                                }	
#endif
