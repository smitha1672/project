...
#define TAS5727_DEVICE_ADDR   0x54
...
byte TAS5727_I2C_read( byte reg_addr)
{
    ...
    {
        ret = I2C2_readSlave(TAS5727_DEVICE_ADDR, reg_addr, &uc, 1, FALSE);

        /*Smith fixes: Don't remove for audio drop*/
...
bool TAS5727_I2C_write(  byte reg_addr, byte value)
{
    ...
    for( i = 0; i < TAS5727_RETRY_TIME; i++ )
    { 
        ret = I2C2_writeSlave(TAS5727_DEVICE_ADDR, reg_addr, (byte*)&value, 1, FALSE);
...
bool TAS5727_I2C_read_n_byte(  byte reg_addr, byte *data, int N)
{
    ...
    for( i = 0; i < TAS5727_RETRY_TIME; i++ )
    { 
        ret = I2C2_readSlave(TAS5727_DEVICE_ADDR, reg_addr, data, N, FALSE);
...
bool TAS5727_I2C_write_n_byte(  byte reg_addr, byte* data,int N)
{
    ...
    for( i = 0; i < TAS5727_RETRY_TIME; i++ )
    {  
        ret = I2C2_writeSlave(TAS5727_DEVICE_ADDR, reg_addr, data, N, FALSE);
