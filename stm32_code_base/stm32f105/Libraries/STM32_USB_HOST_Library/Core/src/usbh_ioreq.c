...
#include "usb_bsp.h"    //by USB_OTG_BSP_mDelay(); use
/** @addtogroup USBH_LIB
  * @{
  */
                             ...
                             uint16_t            length)
{
  ...
  URB_STATE URB_Status ;     //Eason ++
  status = USBH_BUSY;
  USB_OTG_BSP_mDelay(50);   //Eason ++
  URB_Status = HCD_GetURB_State(pdev, phost->Control.hc_num_out); //Eason ++
  ...
  case CMD_WAIT:
    //Eason--if (phost->Control.state == CTRL_COMPLETE ) 
    if( URB_Status ==URB_DONE ) //Eason++
    {
    ...
    else  if( URB_Status ==URB_ERROR ) //Eason++
    {
      /* Failure Mode */
      phost->RequestState = CMD_SEND;
      status = USBH_FAIL;
    ...
    else  if( URB_Status ==URB_STALL ) //Eason++
    {
      /* Commands successfully sent and Response Received  */       
      phost->RequestState = CMD_SEND;
